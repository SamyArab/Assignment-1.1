...
router.get("/students/:id/courses", async (req, res) => {
  try {
    ...
    const student = await Student.findByPk(studentId, {
      include: {
        model: Course,
        as: "Courses", // ✅ Ensure alias matches association
    ...
    console.log(`✅ Fetched courses for student ${studentId}:`, student.Courses)
...
router.get("/students_query_firstName", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error querying students", error: error.message })
  }
})
